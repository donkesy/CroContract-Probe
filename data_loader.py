...
class data_loader:
    ...
    def load_links(self):
        ...
        """
        links = {'total': 0, 'count': Counter(), 'meta': {}, 'data': defaultdict(list), 'edge_type': {}}
        with open(os.path.join(self.path, 'graph_link.txt'), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.split('\t')
                ...
                links['edge_type'][(int(h_id), int(t_id))] = r_id
                # links['edge_type'][(int(t_id), int(h_id))] = r_id
    ...
    def load_nodes(self):
        ...
        """
        nodes = {'total': 0, 'count': Counter(), 'attr': {}, 'type': {}, 'shift': {}}
        with open(os.path.join(self.path, 'graph_node.txt'), 'r', encoding='utf-8') as f:
            for line in f:
                th = line.split('\t')
                if len(th) == 4:
                    ...
                    nodes['attr'][node_id] = np.array(node_attr)
                    ...
                    nodes['count'][node_type] += 1
                    nodes['attr'][node_id] = np.array(node_attr)
                    nodes['type'][node_id] = node_type.split(',')
                    nodes['total'] += 1
                else:
